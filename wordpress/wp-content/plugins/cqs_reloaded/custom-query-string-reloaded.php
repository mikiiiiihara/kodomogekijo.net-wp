...
/* <WP plugin data>
 * Plugin Name:   Custom Query String Reloaded v3
 * Version:       2.9 v3
 * Plugin URI     http://www.transycan.net
 * Description:   Change the number of posts displayed when viewing different archive pages, including tags archives, based on WP's own tag system. Originally by Matt Read http://www.mattread.com/ - Updated for UTW tags by Andy Beard http://andybeard.eu/. 
 * Author         Moshu
 ...
 *
 * 
 * Copyright (C) 2007 Matt Read, Andy Beard and Moshu
 * 
 ...
 * Plugin URI:    http://wordpress.nnn2.com/?p=502
 * Author:        Nori
 * Author URI:    http://wordpress.nnn2.com/
 *
 */

# define the current verion
define('k_CQS_VER', '2.9 v3');

add_action( 'plugins_loaded', 'cqs_load_textdomain' );
function cqs_load_textdomain() {
    load_plugin_textdomain( 'cqs', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}
...
class cqs
{
	...
	function get_category_id($slug) {
		global $wpdb;
		return (int) $wpdb->get_var("SELECT term_id FROM $wpdb->terms WHERE slug = '". $wpdb->escape($slug) ."'");
	}
	 ...
	 */
	function get_slug($category_ID) {
		global $wpdb;
		return $wpdb->get_var("SELECT slug FROM $wpdb->terms WHERE term_id = '". $wpdb->escape((int) $category_ID) ."'");
	}
	
	function get_post_tag($post_tag_ID) {
    global $wpdb;
    return $wpdb->get_var("SELECT post_tag FROM $wpdb->term_taxonomy WHERE term_id = '". $wpdb->escape($post_tag) ."'");
  }
	 ...
	 */
	function footer() {
		echo '<p style="text-align:center;margin-top:3em;">Custom Query String '. k_CQS_VER .' by <a href="http://www.transycan.net/" style="font-family:cursive; font-size:1.1em;">Moshu</a></p><p style="text-align:center;margin-top:3em;">Originally developed by <a href="http://mattread.com" style="font-family:cursive; font-size:1.1em;">Matt Read</a> up until version 2.7 and updated by <a href="http://andybeard.eu" style="font-family:cursive; font-size:1.1em;">Andy Beard</a> v.2.8.</p>';
	}
	...
	<h3><?php _e('Current Conditions', 'cqs'); ?></h3>
		...
		if (strpos($condition, 'cat_') !== false) {
			$term_id = str_replace('cat_', '', $condition);
			$cat_name = $this->get_slug(intval($term_id));
